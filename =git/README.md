...
> 개발자는 변경 내용의 로그 메시지를 남기고 저장소에 commit 한다.  
...
*git의 장점*  
* 수정된 내용을 매번 master 저장소에 commit해서 올리는 대신에 로컬 저장소를 이용한다.  
* 공통 저장소 즉, 중앙 집중식 저장소에 대한 의존성이 없어진다.  
* 개발에 따른 잦은 commit에 유연성을 확보할 수 있다.  
* 보다 안정적인 협업이 이루어 진다.
* 모든 사용자는 완벽한 복사본을 가지고 있다.  
* 중앙 저장소의 사용을 항상 요구하지 않는다.  
* 저장소를 그대로 복사해 올 수 있다. svn과 차이점은 원경 저장소를 복사해 올 수 있지만, 로컬의 저장소를 복사해 올 수도 있다.  

** 직접 작업하는 작업트리 -> 스테이징이라는 인덱스 공간 -> 로컬 저장소(최종 코드 저장소) **
...
*git 설치 및 활용*
1. 윈도우 또는 맥용 git 다운  
	윈도우에서는 GUI(Graphic User Interface)에서 git 명령어를 이용할 수 있지만, 터미널을 이용하여 사용하는 방법을 권장한다.  
1. `> git congig`  
	내가 누구냐?! 정보를 입력하면 이후 commit할 때 해당 이름으로 로그가 남게 된다.  
	git config --global user.name 'name'  
	git config --global user.email 'email'  
1. `> git config --list`  
	git의 사용자 설정 사항을 확인할 수 있다.  
1. git 저장소로 사용할 폴더 생성 후 폴더로 이동 -> `> git init`  
	해당 폴더에는 .git 이라는 숨겨진 디렉토리가 만들어지게 된다.  
1. git 저장소를 사용하고 싶지 않을 경우 ./git 폴더 삭제 -> '> rm -rf .git'  
	탐색기에서 폴더를 휴지통에 버려도 된다.  
1. `> git status`  
	저장소에 대한 정보를 확인할 수 있다. (변경 이력 등)  
1. `> git clone 사용자명@호스트:/원격/저장소/경로` 또는 지정한 폴더에 복사하려면 `> git clone 사용자명@호스트:/원격/저장소/경로/저장될 경로`  
	저장소를 생성했다면 원격 저장소를 복사해 올 수 있다.  
1. `> git log`  
	commit로그을 확인 할 수 있다.  
1. `> git diff 해시코드`  
	이전 파일 버전과 현재 파일 버전의 변경 이력을 확인할 수 있다.  
1. `> git rm 파일명`  
...
** 현재 까지는 branch를 생성하지 않고 master에서 직접 작업한 내용입니다. master는 수정하지 않고 branch를 생성하여 작업 하는 것이 안정적으로 유지보수 할 수 있겠죠? **
...
1. `> git remote add origin 원격서버주소`  
	원격 저장소 등록!  
1. `> git remote`  
...
*용어*  
* 저장소(repository) - 사용자가 변경한 모든 내용을 추적 가능한 공간이다. 저장소는 히스토리에 따른 다른 버전들을 전부 가지고 있다. 다른 곳으로 복사하여도 히스토리 또한 복사되므로 완벽한 저장소를 유지한다.  
* 분기(branch) - 모든 분기는 master에서 이루어 진다.  
* 커밋(commit) - 모든 히스토리를 로컬 저장소로 커밋할 수 있다.  
* 리비전(revision) - 소스코드의 버전을 표현한다.  
* 오리진(origin) - 단지 git이 복사해 온 저장소를 가리키기 위해 기본적으로 사용하는 이름이다.  
...
** 정적인 웹페이지를 호스팅을 사용하지 않고 서비스 하기 **
** gh-pages는 github에서 지원하는 서비스 : 서버 호스팅 없이 사용 가능하도록 지원해 준다. 예를 들어 test 라는 저장소를 github에 생성하고 git branch gh-pages라는 커멘드로 branch를 생성하고 html 을 푸시하면 http://{account}.github.com/test로 접속할 경우 gh-pages의 branch로 푸시한 html 이 보여지게 된다. **  
** {account}.github.com 저장소를 생성해 gh-pages를 사용하지 않아도 github 시스템이 자동으로 {account}.github.com 이라는 도메인으로 해당 저장소에 있는 정적 html 을 웹페이지 서비스를 제공해 준다. **  
